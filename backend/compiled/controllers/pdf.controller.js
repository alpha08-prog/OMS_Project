...
exports.generateTrainEQPDF = generateTrainEQPDF;
exports.generateGrievancePDF = generateGrievancePDF;
exports.generateTourProgramPDFController = generateTourProgramPDFController;
exports.previewTrainEQ = previewTrainEQ;
exports.previewGrievance = previewGrievance;
 ...
 */
async function generateTrainEQPDF(req, res) {
    try {
        const { id } = req.params;
        ...
        if (req.user?.role === 'STAFF' && trainRequest.createdById !== req.user.id) {
            (0, response_1.sendForbidden)(res, 'You can only download your own train EQ letters');
            return;
        }
        ...
        (0, pdfGenerator_1.generateTrainEQLetter)({
            refNumber,
        ...
        (0, response_1.sendServerError)(res, 'Failed to generate Train EQ PDF', error);
    }
}
/**
 ...
 */
async function generateGrievancePDF(req, res) {
    try {
        const { id } = req.params;
        ...
        if (req.user?.role === 'STAFF' && grievance.createdById !== req.user.id) {
            (0, response_1.sendForbidden)(res, 'You can only download your own grievance letters');
            return;
        }
        ...
        (0, pdfGenerator_1.generateGrievanceLetter)({
            refNumber,
            date: new Date().toLocaleDateString('en-IN', {
                day: '2-digit',
                month: 'long',
        ...
        (0, response_1.sendServerError)(res, 'Failed to generate Grievance PDF', error);
    }
}
/**
 ...
 */
async function generateTourProgramPDFController(req, res) {
    try {
        ...
        const events = await prisma_1.default.tourProgram.findMany({
            where,
            orderBy: { dateTime: 'asc' },
        });
        if (events.length === 0) {
            (0, response_1.sendError)(res, 'No accepted events found for the specified period', 404);
            return;
        }
        ...
        (0, pdfGenerator_1.generateTourProgramPDF)(events.map((e) => ({
        ...
        (0, response_1.sendServerError)(res, 'Failed to generate Tour Program PDF', error);
    }
}
/**
 ...
 */
async function previewTrainEQ(req, res) {
    try {
        const { id } = req.params;
        const trainRequest = await prisma_1.default.trainRequest.findUnique({
            where: { id },
        ...
        if (req.user?.role === 'STAFF' && trainRequest.createdById !== req.user.id) {
            (0, response_1.sendForbidden)(res, 'You can only preview your own train EQ letters');
            return;
        }
        const refNumber = `EQ/${new Date().getFullYear()}/${trainRequest.id.slice(0, 8).toUpperCase()}`;
...
<body>
  ...
  <div class="footer">
    ...
    `;
        ...
        (0, response_1.sendServerError)(res, 'Failed to preview letter', error);
    }
}
/**
 ...
 */
async function previewGrievance(req, res) {
    try {
        const { id } = req.params;
        const grievance = await prisma_1.default.grievance.findUnique({
            where: { id },
        ...
        if (req.user?.role === 'STAFF' && grievance.createdById !== req.user.id) {
            (0, response_1.sendForbidden)(res, 'You can only preview your own grievance letters');
            return;
        }
        // Determine department based on grievance type
        const departmentMap = {
