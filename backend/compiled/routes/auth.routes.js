...
const createUserValidation = [
    (0, express_validator_1.body)('name').trim().notEmpty().withMessage('Name is required'),
    (0, express_validator_1.body)('email').isEmail().normalizeEmail().withMessage('Valid email is required'),
    (0, express_validator_1.body)('phone').optional({ nullable: true, checkFalsy: true }).matches(/^\d{10}$/).withMessage('Phone must be 10 digits'),
    (0, express_validator_1.body)('password').isLength({ min: 8 }).withMessage('Password must be at least 8 characters'),
    (0, express_validator_1.body)('role').isIn(['STAFF', 'ADMIN', 'SUPER_ADMIN']).withMessage('Invalid role'),
];
router.get('/users', auth_1.authenticate, auth_1.adminOnly, auth_controller_1.getAllUsers);
router.post('/users', auth_1.authenticate, auth_1.adminOnly, (0, validate_1.validate)(createUserValidation), auth_controller_1.createUser);
