...
let cachedEmblem;
function getEmblemBuffer() {
    if (cachedEmblem !== undefined)
        return cachedEmblem;
    const candidates = [
        path_1.default.resolve(__dirname, '../assets/Embelem.png'),
        path_1.default.resolve(__dirname, '../../src/assets/Embelem.png'),
        path_1.default.resolve(process.cwd(), 'src/assets/Embelem.png'),
        path_1.default.resolve(process.cwd(), 'backend/src/assets/Embelem.png'),
    ];
    for (const p of candidates) {
        try {
            if (fs_1.default.existsSync(p)) {
                cachedEmblem = fs_1.default.readFileSync(p);
                return cachedEmblem;
            }
        }
        catch {
...
function createWatermark(doc, documentId, refNumber) {
...
function createFooter(doc) {
    ...
    doc.fontSize(8)
        .fillColor(COLORS.gray)
        .text('Office of Hon\'ble Minister | Krishi Bhawan, New Delhi - 110001 | Tel: 011-23383615', margin, footerY + 10, { align: 'center', width: pageWidth - margin * 2 });
}
...
function generateTrainEQLetter(data, res) {
    ...
    if (data.passengerDetails && data.passengerDetails.length > 0) {
        ...
        const splitNames = data.passengerName
            ...
            .map((n) => n.trim())
            .filter(Boolean);
        const names = splitNames.length > 1 ? splitNames : data.passengerName.trim() ? [data.passengerName.trim()] : [];
    ...
    streamPdfToResponse(res, filename, 'TrainEQ', (doc) => {
        ...
        if (rows.length === 0) {
            ...
            rows.forEach((p, i) => {
                ...
                if (sa)
                    doc.text(sa, colX[2] + 4, y, { width: colW[2] - 8, lineBreak: false });
