...
import { Cake, Send, ChevronDown, ChevronUp } from "lucide-react";
    ...
    <Card className="shadow-sm border-pink-200 bg-gradient-to-br from-pink-50 to-rose-50 dark:from-pink-950/20 dark:to-rose-950/20 dark:border-pink-800">
      ...
      <CardContent className="space-y-3">
        {loading ? (
          ...
          <>
            {(showAll ? birthdays : birthdays.slice(0, limit)).map((person) => (
              <div
                key={person.id}
            ...
            {birthdays.length > limit && (
              <Button
                variant="ghost"
                size="sm"
                className="w-full mt-2 text-pink-600 hover:text-pink-700 hover:bg-pink-100 dark:hover:bg-pink-900/30"
                onClick={() => setShowAll(!showAll)}
              >
                {showAll ? (
                  <>Show Less <ChevronUp className="ml-2 h-4 w-4" /></>
                ) : (
                  <>Show All ({birthdays.length}) <ChevronDown className="ml-2 h-4 w-4" /></>
                )}
              </Button>
            )}
          </>
